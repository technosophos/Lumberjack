...
Config::request('default')
  ...
  ->doesCommand('echoTableHeader')
    ->whichInvokes('FortissimoEcho')
    ->withParam('text')->whoseValueIs('<table id="logdata">
      <thead><tr><th>Date</th><th>Type</th><th>Message</th><th>Facility</th><tr></thead><tbody>')
  ->doesCommand('recent')
    ->whichInvokes('SimpleLogQuery')
    ->withParam('filter')->whoseValueIs(array('type' => 'drupal'))
    ->withParam('sort')->whoseValueIs(array('date' => 1))
    ->withParam('limit')->whoseValueIs(500)
    ->withParam('skip')->whoseValueIs(0)
    ->withParam('fields')->whoseValueIs(array('date', 'type', 'message', 'facility'))
  ->doesCommand('printEachRow')
    ->whichInvokes('PrintEachRow')
    ->withParam('iterable')->from('cxt:recent')
    //->withParam('format')->whoseValueIs('<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>')
    ->withParam('formatter')->whoseValueIs('log_table_row')
    ->withParam('fields')->whoseValueIs(array('date', 'type', 'message', 'facility'))
  ->doesCommand('echoTableFooter')
    ->whichInvokes('FortissimoEcho')
    ->withParam('text')->whoseValueIs('</tbody></table>')  
